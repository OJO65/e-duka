...
export class RegisterComponent implements OnInit {
  ...
  onPasswordInput(): void { this.passwordStrength = this.calcStrength(this.password); }
  ...
  onSubmit(): void {
    ...
    if (this.username.length < 3) {
      this.errorMessage = 'Username must be at least 3 characters'; return;
    }
    if (!this.isValidEmail(this.email)) {
      this.errorMessage = 'Please enter a valid email address'; return;
    }
    if (this.password.length < 8) {
      this.errorMessage = 'Password must be at least 8 characters'; return;
    }
    if (this.password !== this.confirmPassword) {
      this.errorMessage = 'Passwords do not match'; return;
    }
    if (!this.agreeToTerms) {
      this.errorMessage = 'Please agree to the Terms and Conditions'; return;
    }
  ...
  goToLogin(): void { this.router.navigate(['/login'], { queryParams: { returnUrl: this.returnUrl } }); }
  ...
  getPasswordStrengthColor(): string {
    const map: any = { weak: 'bg-red-500', medium: 'bg-yellow-500', strong: 'bg-green-500' };
