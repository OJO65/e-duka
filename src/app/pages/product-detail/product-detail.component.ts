...
export class ProductDetailComponent implements OnInit, OnDestroy {
  ...
  loadProduct(): void {
    ...
    this.productService.getProductById(this.productId).subscribe({
      next: (result: any) => {
        ...
        if (!this.product) { this.router.navigate(['/']); return; }
        ...
        const formattedPrice = 'KES ' + new Intl.NumberFormat('en-KE', { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(price);
        const description = `Buy ${this.product.title} at GNET Computers Kenya. ${formattedPrice}. ${this.product.description?.slice(0, 100) || ''}`;

        this.titleService.setTitle(`${this.product.title} — GNET Computers Kenya`);
        this.meta.updateTag({ name: 'description', content: description });
        this.meta.updateTag({ property: 'og:title', content: `${this.product.title} — GNET Computers Kenya` });
        this.meta.updateTag({ property: 'og:description', content: description });
        this.meta.updateTag({ property: 'og:image', content: this.product.images?.nodes?.[0]?.url || '' });
  ...
  selectVariant(variant: any): void {
    ...
    if (variant.image?.url) {
      const index = this.product.images.nodes.findIndex((img: any) => img.url === variant.image.url);
